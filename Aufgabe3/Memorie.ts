...
function randomState(): string {
    ...
    if (randomState <= .5) {
        return "hidden";
        //50%ige Chance, dass die Karte verdeckt ist
        
    }else if (randomState > .5 && randomState <= .75) {
        return "taken";
        //Wenn die Wahrscheinlichkeit größer 50% und kleiner gleich 75% ist dann wird die Karte als genommen deklariert
        
    }else if (randomState > .75) {
        return "visible";
        //Wenn die Wahrscheinlichkeit größer 75% ist gilt die Karte als als aufgedeckt
